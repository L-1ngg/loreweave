...
async function resolveModelOptions(options: PiPortOptions): Promise<ModelPortOptions> {
	...
	return { ...options, sessionId: options.sessionId ?? randomUUID(), model, stream: (model, context, settings) => {
		settings?.signal?.throwIfAborted();
		options.beforeModelRequest?.({ kind: context.systemPrompt === SUMMARY_SYSTEM ? "summary" : "task", ...(settings?.signal ? { signal: settings.signal } : {}) });
